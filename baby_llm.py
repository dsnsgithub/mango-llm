...
output_projection  = nn.Linear(EMBEDDING_DIMENSION, vocabulary_size)
...
# ── Training ──────────────────────────────────────────────────────
causal_mask = nn.Transformer.generate_square_subsequent_mask(CONTEXT_LENGTH)
...
for epoch_number in range(NUMBER_OF_EPOCHS):
    ...
    if (epoch_number + 1) % 500 == 0:
        ...
        with torch.no_grad():
            ...
            validation_embeddings = token_embedding(validation_input_sequences) + position_embedding(torch.arange(CONTEXT_LENGTH))
...
def generate_text(prompt_string, max_new_tokens=200):
    ...
    for _ in range(max_new_tokens):
        ...
        cropped_embeddings   = token_embedding(cropped_indices) + position_embedding(torch.arange(cropped_indices.shape[1]))
        cropped_hidden       = transformer_encoder(cropped_embeddings, mask=nn.Transformer.generate_square_subsequent_mask(cropped_indices.shape[1]), is_causal=True)
